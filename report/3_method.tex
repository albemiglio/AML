...
Because standard pre-trained CNNs expect a 3-channel input, we replicate the single-channel depth map across all three channels before passing it through a ResNet-18~\cite{he2016deep} backbone (\emph{Main} variant), yielding a 512D global average pooling vector $\mathbf{f}_{\text{depth}} \in \R^{512}$.
This channel-replication trick allows the use of ImageNet weights even for a fundamentally different modality, enabling the network to leverage pre-trained low-level filters (Sobel-like edge detectors) that naturally capture depth discontinuities and surface normal information encoded as intensity gradients in the depth image.
...
In the \emph{Extension} variant, we replace ResNet-18 with a custom single-channel ResNet-like backbone consisting of four residual blocks trained from scratch, preserving the original single-channel input and avoiding any domain mismatch introduced by channel replication.
